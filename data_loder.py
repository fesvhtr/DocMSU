...
class STDataset(Dataset):
    ...
    def __generate_text_label_mask(self, textboxes):
        ...
        for row, labels in enumerate(textboxes): # labels是其中一行文本的textbox，len(labels)表示其中有几段讽刺
            ...
            textboxes[row] = labels  # textboxes每一行长度为4：例如[[0, 2], [27, 29],[-1, -1], [-1, -1]], [[18], [-1, -1], [-1, -1], [-1, -1]]
    ...
    def __getitem__(self, idx):
        ...
        return self.ids[idx], self.text_mask[idx], self.textboxes[idx], img_tensor, self.imgboxes[idx], torch.tensor(1), label

    def __len__(self):
        return len(self.obj_list)


def read_pic(img_label_root_path):
    img_label_path_list = [os.path.join(img_label_root_path, i) for i in os.listdir(img_label_root_path)]
    total_obj_list = []
    last_end_idx = 0
    for img_label_path in img_label_path_list:
        temp_obj_list = []
        with open(img_label_path, 'r+', encoding='utf-8') as f:
            for item in jsonlines.Reader(f):
                filename = str(int(item['filename'].split(".")[0]) + last_end_idx) + ".jpg"   # 数据集中的img_name,如243.jpg
                # bbox为列表，里面是嵌套的字典，包含x,y,width,height, label等key，列表里可能有多个字典(多个讽刺对象)
                value = item['bbox']  # 检测的讽刺对象的bounding box坐标，如[{"uuid":"7cd348f6-7438-4e71-845c-efb82035836b","x":42.0174007803,"y":4.0852355957,"width":290.0023567024,"height":194.0,"label":"person"}]
                # 如果没有标注bbox，说明图片无讽刺
                if len(value) == 0:
                    ...
                    temp_obj_list.append(obj)
        temp_obj_list.sort(key=lambda obj: int(obj.filename.split(".")[0])) # 根据图片序号排序，从小到大
        total_obj_list += temp_obj_list
        last_end_idx = len(total_obj_list)

    return total_obj_list

def read_text(text_root_path, total_obj_list):
    text_path_list = [os.path.join(text_root_path, i) for i in os.listdir(text_root_path)]
    temp_idx = 0
    for text_path in text_path_list:
        with open(text_path, 'r+', encoding='utf-8') as f:
            for item in jsonlines.Reader(f):
                filename, text = item['text'].split('\t')[0][:-1], item['text'].split('\t')[1]   # filename: text形式的数字，如1， 2， 3，
                labels = item['label']  # 已经优化过的以word为索引的label， 如：[[0, 1, 'organization'], [10, 14, 'organization']]
                obj = total_obj_list[temp_idx]
                obj.text = text
                if len(labels) != 0:
                    obj.textbox = [[label[0], label[1]] if len(label) == 3 else [label[0], label[0]] for label in labels] # 这里有一个问题，就是text里的类别没有用到
...
def word2vec(sentences, labels, mask, if_text_detection=True):
    ...
    for sent in sentences:
        ...
        sent_len = torch.count_nonzero(encoded_dict['input_ids'], dim=1).reshape(-1)   # 分词后句子中不为0的idx的长度(多包含了cls和sep的idx)，这个reshape(-1)好像没什么用？
...
def read_from_zip(img_root_path, total_obj_list):
    img_path_list = [os.path.join(img_root_path, i) for i in os.listdir(img_root_path)]
    temp_idx = 0
    for img_class_path in img_path_list:
        name_list = os.listdir(img_class_path)
...
def load_data(img_label_src, text_label_src, img_src, batch_size, train_split_ratio=0.7):
    start = time.time()
    res = read_from_zip(img_src, read_text(text_label_src, read_pic(img_label_src)))
...
def load_data2(json_src, batch_size, train_split_ratio=0.7):
